AFRAME.registerComponent('tower-menu', {
    ...
    updateMenu: function() {
        const data = TOWER_DATA[this.selectedTower];
        ...
        const miniModel = document.createElement('a-entity');
...
AFRAME.registerComponent('ar-game-controller', {
    ...
    checkInputSources: async function(session) {
        ...
        const targetSource = sources.find(s => s.targetRayMode === 'tracked-pointer' && s.handedness === 'right')
            || sources.find(s => s.targetRayMode === 'tracked-pointer');
    ...
    tick: function () {
        ...
        if (!this.cameraEl) this.cameraEl = document.querySelector('a-camera');
        ...
        if (frame) {
            ...
            if (results.length > 0) {
                ...
                const rayPose = frame.getPose(this.inputSource.targetRaySpace, refSpace);
                if (rayPose) {
                    const cp = rayPose.transform.position;
                    const dRaySq = Math.pow(cp.x - hitPos.x, 2) + Math.pow(cp.y - hitPos.y, 2) + Math.pow(cp.z - hitPos.z, 2);
                    if (dRaySq < 0.04) isValid = false;
                }

                if (this.cameraEl && isValid) {
                    const camPos = this.cameraEl.object3D.position;
                    const dCamSq = Math.pow(camPos.x - hitPos.x, 2) + Math.pow(camPos.z - hitPos.z, 2);
                    if (dCamSq < 0.04) isValid = false;
                }

                const gameSystem = this.el.sceneEl.systems['game-manager'];
                if (gameSystem && gameSystem.gameState === 'playing' && isValid) {
                    const basePos = gameSystem.basePosition;
                    const dBaseSq = Math.pow(basePos.x - hitPos.x, 2) + Math.pow(basePos.z - hitPos.z, 2);
                    if (dBaseSq < 0.16) isValid = false;

                    const towers = gameSystem.towers;
                    for (let i = 0; i < towers.length; i++) {
                        const tPos = towers[i].object3D.position;
                        const dTtSq = Math.pow(tPos.x - hitPos.x, 2) + Math.pow(tPos.z - hitPos.z, 2);
                        if (dTtSq < 0.16) {
                            isValid = false;
                            if (dTtSq < 0.09) hoveredTower = towers[i];
                        }
                    }
                }
                ...
                if (hoveredTower) {
                    const tLogic = hoveredTower.components['tower-logic'];
                    if (tLogic && tLogic.canUpgrade()) color = '#0088FF';
                }
        ...
        if (this.reticle.getAttribute('visible') === true) {
            ...
            if (gameSystem.gameState === 'placing_base' && this.isValidPlacement) {
                ...
                if (this.hoveredTower) {
                    const tLogic = this.hoveredTower.components['tower-logic'];
                    if (tLogic && tLogic.canUpgrade()) {
                        const cost = tLogic.getUpgradeCost();
                        if (gameSystem.tryBuyTower(cost)) {
                            tLogic.upgrade();
                        }
                    }
                } else if (this.isValidPlacement) {
                    const towerType = gameSystem.selectedTowerType;
